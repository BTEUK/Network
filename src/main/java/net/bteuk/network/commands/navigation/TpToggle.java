...
public class TpToggle extends AbstractCommand {
    ...
    public void execute(@NotNull CommandSourceStack stack, @NotNull String[] args) {
        ...
        if (Network.getInstance().getGlobalSQL()
                .hasRow("SELECT uuid FROM player_data WHERE uuid='" + player.getUniqueId() + "' AND teleport_enabled" +
                        "=1;")) {
            // Disable teleport.
            user.setTeleportEnabled(false);
            Network.getInstance().getGlobalSQL()
                    .update("UPDATE player_data SET teleport_enabled=0 WHERE uuid='" + player.getUniqueId() + "';");
            ...
            Network.getInstance().getGlobalSQL()
                    .update("UPDATE player_data SET teleport_enabled=1 WHERE uuid='" + player.getUniqueId() + "';");
