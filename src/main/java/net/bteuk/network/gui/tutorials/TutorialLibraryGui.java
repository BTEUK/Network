    ...
    private void setActions() {
        ...
        if (tutorialsUser.hasIncompleteLessons(Network.getInstance().getTutorialsDBConnection(), LOGGER)) {
            ...
            if (iTutorialIDCurrentLesson == -1) {
                Bukkit.getConsoleSender().sendMessage(Utils.error("An error occurred. Player is in lesson but has no " +
                        "lesson in the database"));
            }
            Tutorial currentTutorial = Tutorial.fetchByTutorialID(iTutorialIDCurrentLesson,
                    Network.getInstance().getTutorialsDBConnection(), LOGGER);
            ...
            for (i = 0; i < inUseTutorials.length; i++) {
                ...
                setAction(iSlot, user -> {
                    ...
                    Bukkit.getConsoleSender()
                            .sendMessage("TutorialID of slot: " + inUseTutorials[iSlot].getTutorialID());
                    if (tutorialsUser.hasIncompleteLessons(Network.getInstance().getTutorialsDBConnection(),
                            LOGGER) && (currentTutorial.getTutorialID() != inUseTutorials[iSlot].getTutorialID())) {
                        user.sendMessage(Utils.error("You cannot start a new tutorial before you finish your current " +
                                "one"));
                    } else {
                        if (Event.addEvent(EventType.LIBRARY, user.player.getUniqueId(),
                                inUseTutorials[iSlot].getTutorialID(),
                                Network.getInstance().getTutorialsDBConnection(), LOGGER))
