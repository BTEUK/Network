...
public record Region(String regionName, int x, int z) {
    ...
    public String getTag(String uuid) {
        if (hasTag(uuid)) {
            return Network.getInstance().regionSQL.getString(
                    "SELECT tag FROM region_members WHERE region='" + regionName + "' AND uuid='" + uuid + "';");
        } else {
            return regionName;
        }
    }
    ...
    public void setTag(String uuid, String tag) {
        Network.getInstance().regionSQL.update("UPDATE region_members SET tag='" + tag.replace("'", "\\'") + "' WHERE" +
                " region='" + regionName + "' AND uuid='" + uuid + "';");
    }

    // Return whether the region has a tag for the specified uuid.
    private boolean hasTag(String uuid) {
        return (Network.getInstance().regionSQL.hasRow(
                "SELECT region FROM region_members WHERE region='" + regionName + "' AND uuid='" + uuid + "' AND tag " +
                        "IS NOT NULL;"));
    }
    ...
    public boolean isPinned(String uuid) {
        return Network.getInstance().regionSQL.hasRow(
                "SELECT region FROM region_members WHERE region='" + regionName + "' AND uuid='" + uuid + "' AND " +
                        "pinned=1;");
    }

    // Set the region to (un)pinned for a specific player.
    public void setPinned(String uuid, boolean pin) {
        Network.getInstance().regionSQL.update("UPDATE region_members SET pinned=" + (pin ? "1" : "0") + " WHERE " +
                "region='" + regionName + "' AND uuid='" + uuid + "';");
    }

    // Get the server of the region.
    public String getServer() {
        if (Network.getInstance().regionSQL.hasRow("SELECT region FROM regions WHERE region='" + regionName + "' AND " +
                "status='plot'")) {
            // Region is on a plot server.
            return (Network.getInstance().getPlotSQL()
                    .getString("SELECT server FROM regions WHERE region='" + regionName + "';"));
        } else {
    ...
    public RegionStatus status() {
        String status =
                Network.getInstance().regionSQL.getString("SELECT status FROM regions WHERE region='" + regionName +
                        "';");
        if (status == null) {
    ...
    public boolean wasClaimed() {
        return (Network.getInstance().regionSQL.hasRow(
                "SELECT region FROM region_logs WHERE region='" + regionName + "' AND is_owner=1;"));
    }
    ...
    public void setPlot() {
        ...
        for (String uuid : getMembers()) {
            ...
            Network.getInstance().regionSQL.update("DELETE FROM region_members WHERE region='" + regionName + "' AND " +
                    "uuid='" + uuid + "';");
    ...
    public boolean hasOwner() {
        return (Network.getInstance().regionSQL.hasRow(
                "SELECT region FROM region_members WHERE region='" + regionName + "' AND is_owner=1;"));
    }

    // Return whether the region has an owner.
    public boolean hasActiveOwner() {
        return (Network.getInstance().regionSQL.hasRow(
                "SELECT region FROM region_members WHERE region='" + regionName + "' AND is_owner=1 AND last_enter>=" + (Time.currentTime() - Network.getInstance()
                        .getTimers().inactivity) + ";"));
    }

    // Return whether the region has a member.
    public boolean hasMember() {
        return (Network.getInstance().regionSQL.hasRow(
                "SELECT region FROM region_members WHERE region='" + regionName + "' AND is_owner=0;"));
    }

    // Return whether the region has an active member.
    public boolean hasActiveMember(long time) {
        return (Network.getInstance().regionSQL.hasRow(
                "SELECT region FROM region_members WHERE region='" + regionName + "' AND is_owner=0 AND last_enter>=" + time + ";"));
    }

    // Return the number of members, if any.
    public int memberCount() {
        return (Network.getInstance().regionSQL.getInt(
                "SELECT COUNT(uuid) FROM region_members WHERE region='" + regionName + "' AND is_owner=0;"));
    }
    ...
    public String getOwner() {
        if (hasOwner()) {
            return (Network.getInstance().regionSQL.getString(
                    "SELECT uuid FROM region_members WHERE region='" + regionName + "' AND is_owner=1;"));
        } else {
            return "null";
        }
    }
    ...
    public String ownerName() {
        if (hasOwner()) {
            return (Network.getInstance().getGlobalSQL()
                    .getString("SELECT name FROM player_data WHERE uuid='" + getOwner() + "';"));
        } else {
            return "nobody";
        }
    }
    ...
    public ArrayList<String> getMembers() {
        return Network.getInstance().regionSQL.getStringList(
                "SELECT uuid FROM region_members WHERE region='" + regionName + "';");
    }

    // Get the coordinate id for the location the player has set.
    public int getCoordinateID(String uuid) {
        return Network.getInstance().regionSQL.getInt(
                "SELECT coordinate_id FROM region_members WHERE region='" + regionName + "' AND uuid='" + uuid + "';");
    }

    // Set the coordinate id for the location of the specified player.
    public void setCoordinateID(String uuid, int id) {
        Network.getInstance().regionSQL.update(
                "UPDATE region_members SET coordinate_id=" + id + " WHERE region='" + regionName + "' AND uuid='" + uuid + "';");
    }

    // Set the last enter time for the region.
    public void setLastEnter(String uuid) {
        Network.getInstance().regionSQL.update("UPDATE region_members SET last_enter=" + Time.currentTime() + " WHERE" +
                " region='" + regionName + "' AND uuid='" + uuid + "';");
    }
    ...
    public String getRecentMember() {
        if (hasMember()) {
            return Network.getInstance().regionSQL.getString(
                    "SELECT uuid FROM region_members WHERE region='" + regionName + "' AND is_owner=0 ORDER BY " +
                            "last_enter DESC;");
        } else {
            return null;
        }
    ...
    public void addToDatabase() {
        ...
        if (!inDatabase()) {
            Network.getInstance().regionSQL.update("INSERT INTO regions(region,status) VALUES('" + regionName + "'," +
                    "'default');");

            // Create region in worldguard.
            WorldGuard.createRegion(regionName, Integer.parseInt(regionName.split(",")[0]) * 512,
                    Integer.parseInt(regionName.split(",")[1]) * 512,
                    Integer.parseInt(regionName.split(",")[0]) * 512 + 511,
                    Integer.parseInt(regionName.split(",")[1]) * 512 + 511,
    ...
    public boolean hasRequests() {
        return Network.getInstance().regionSQL.hasRow(
                "SELECT region FROM region_requests WHERE region='" + regionName + "';");
    }

    // Check if the player has already requested to join this region.
    public boolean hasRequest(NetworkUser u) {
        return Network.getInstance().regionSQL.hasRow(
                "SELECT region FROM region_requests WHERE region='" + regionName + "' AND uuid='" + u.player.getUniqueId() + "';");
    }

    // Check if the specified player has been invited to this region.
    public boolean hasInvite(String uuid) {
        return Network.getInstance().regionSQL.hasRow(
                "SELECT region FROM region_invites WHERE region='" + regionName + "' AND uuid='" + uuid + "';");
    }

    // Remove a region invite.
    public void removeInvite(String uuid) {
        Network.getInstance().regionSQL.update("DELETE FROM region_invites WHERE region='" + regionName + "' AND " +
                "uuid='" + uuid + "';");
    }
    ...
    public void acceptRequests() {
        ...
        ArrayList<String> uuids = Network.getInstance().regionSQL.getStringList("SELECT uuid FROM region_requests " +
        ...
        for (String uuid : uuids) {
            ...
            coordinate_id = Network.getInstance().regionSQL.getInt("SELECT coordinate_id FROM region_requests WHERE " +
                    "region='" + regionName + "' AND uuid='" + uuid + "';");
    ...
    public void acceptRequest(String uuid) {
        ...
        int coordinate_id = Network.getInstance().regionSQL.getInt("SELECT coordinate_id FROM region_requests WHERE " +
                "region='" + regionName + "' AND uuid='" + uuid + "';");
        ...
        // Delete request.
        Network.getInstance().regionSQL.update("DELETE FROM region_requests WHERE region='" + regionName + "' AND " +
                "uuid='" + uuid + "'; ");
    }
    ...
    public void requestRegion(NetworkUser u, boolean staffRequest) {
        ...
        if (staffRequest) {
            ...
            // Create request.
            Network.getInstance().regionSQL.update("INSERT INTO region_requests(region,uuid,owner,staff_accept," +
                    "coordinate_id) VALUES ('" + regionName + "','" +
            ...
            ChatMessage chatMessage = new ChatMessage(ChatChannels.REVIEWER.getChannelName(), "server",
                    ChatUtils.success("A region join request has been submitted by %s for region %s",
                            u.player.getName(), regionName));
            Network.getInstance().getChat().sendSocketMessage(chatMessage);
        } else {
            // Owner request

            // Create request.
            Network.getInstance().regionSQL.update("INSERT INTO region_requests(region,uuid,owner,owner_accept," +
                    "coordinate_id) VALUES ('" + regionName + "','" +
                    u.player.getUniqueId() + "','" + getOwner() + "',0," + coordinate + ");");
    ...
    public void joinRegion(NetworkUser u) {
        ...
        if (status() == RegionStatus.PUBLIC) {
            ...
            Network.getInstance().regionSQL.update("INSERT INTO region_members(region,uuid,last_enter,coordinate_id) " +
                    "VALUES('" + regionName + "','" +
                    u.player.getUniqueId() + "'," + Time.currentTime() + "," + coordinateID + ");");

            // Start log of player in region.
            Network.getInstance().regionSQL.update(
                    "INSERT INTO region_logs(region,uuid,start_time) VALUES('" + regionName + "','" +
            ...
            if (status() == RegionStatus.INACTIVE) {
                ...
                Network.getInstance().regionSQL.update(
                        "UPDATE region_members SET is_owner=0 WHERE region='" + regionName + "' AND uuid='" + owner + "';");

                // Close log for owner.
                Network.getInstance().regionSQL.update("UPDATE region_logs SET end_time=" + Time.currentTime() + " " +
                        "WHERE region='" + regionName + "' AND uuid='" + owner + "';");
            ...
            Network.getInstance().regionSQL.update("INSERT INTO region_members(region,uuid,is_owner,last_enter," +
                    "coordinate_id) VALUES('" + regionName + "','" +
                    u.player.getUniqueId() + "',1," + Time.currentTime() + "," + coordinateID + ");");

            // Start log of player in region.
            Network.getInstance().regionSQL.update("INSERT INTO region_logs(region,uuid,is_owner,start_time) VALUES" +
                    "('" + regionName + "','" +
    ...
    public void leaveRegion(String uuid, Component message) {
        ...
        if (SERVER_TYPE == ServerType.EARTH) {
            ...
            EventManager.createEvent(uuid, "network", Network.getInstance().getGlobalSQL().getString("SELECT name " +
    ...
    public void makeMember() {
        ...
        Network.getInstance().regionSQL.update("UPDATE region_members SET is_owner=0 WHERE region='" + regionName +
                "' AND uuid='" + uuid + "';");
    }
    ...
    public void makeOwner(String uuid) {
        ...
        if (isMember(uuid)) {
            ...
            Network.getInstance().regionSQL.update(
                    "UPDATE region_members SET is_owner=1 WHERE region='" + regionName + "' AND uuid='" + uuid + "';");
    ...
    public void updateRequests() {
        ...
        if (hasOwner()) {
            ...
            Network.getInstance().regionSQL.update("UPDATE region_requests SET owner='" + getOwner() + "' WHERE " +
                    "region='" + regionName + "';");
        } else {
    ...
    public void removeMembers(String message, boolean success) {
        ...
        if (hasOwner() || hasMember()) {
            ...
            // Get all members.
            ArrayList<String> uuids = Network.getInstance().regionSQL.getStringList("SELECT uuid FROM region_members " +
                    "WHERE region='" + regionName + "';");

            for (String uuid : uuids) {

                leaveRegion(uuid, success ? ChatUtils.success(message, getTag(uuid)) : ChatUtils.error(message,
                        getTag(uuid)));
            }
        }
    }
    ...
    public long lastActive(String uuid) {
        return (Network.getInstance().regionSQL.getLong(
                "SELECT last_enter FROM region_members WHERE region='" + regionName + "' AND uuid='" + uuid + "';"));
    }

    // Check if the player can build in this region.
    public boolean canBuild(Player p) {
        return ((status() == RegionStatus.OPEN && p.hasPermission("group.jrbuilder")) || isOwner(
                p.getUniqueId().toString()) || isMember(p.getUniqueId().toString()));
    }
