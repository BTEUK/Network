        ...
        switch (event[1]) {
                ...
                if (player != null) {
                    ...
                    p.sendMessage(ChatUtils.success("Teleported to ")
                            .append(Component.text(Network.getInstance().getGlobalSQL().getString("SELECT name FROM " +
                                    "player_data WHERE uuid='" + event[2] + "';"), NamedTextColor.DARK_AQUA)));
                } else {
                    p.sendMessage(Component.text(Network.getInstance().getGlobalSQL().getString("SELECT name FROM " +
                                    "player_data WHERE uuid='" + event[2] + "';"), NamedTextColor.DARK_RED)
            ...
            case "location", "location_request" -> {
                ...
                if (event[1].equals("location")) {
                    coordinate_id = Network.getInstance().getGlobalSQL().getInt("SELECT coordinate FROM location_data" +
                            " WHERE location='" + location + "';");
                } else {
                    coordinate_id = Network.getInstance().getGlobalSQL().getInt("SELECT coordinate FROM " +
                            "location_requests WHERE location='" + location + "';");
                }

                Location l = Network.getInstance().getGlobalSQL().getLocation(coordinate_id);

                String worldName = Network.getInstance().getGlobalSQL().getString("SELECT world FROM coordinates " +
                ...
                if (Network.getInstance().getPlotSQL()
                    ...
                    l = new Location(
                            ...
                            l.getX() + Network.getInstance().getPlotSQL().getInt("SELECT xTransform FROM " +
                                    "location_data WHERE name='" + worldName + "';"),
                            l.getY(),
            ...
            case "region" -> {
                ...
                if (l == null) {
                    ...
                    Network.getInstance().getLogger()
                            .warning("Location is null for coodinate id " + region.getCoordinateID(uuid));
                    return;
                }
            ...
            case "spawn" -> {
                ...
                if (SERVER_TYPE == ServerType.LOBBY) {
                    ...
                    EventManager.createTeleportEvent(true, p.getUniqueId().toString(), "network", "teleport spawn",
                            p.getLocation());
                    SwitchServer.switchServer(p, Network.getInstance().getGlobalSQL().getString("SELECT name FROM " +
